...
$$
   DECODE(INPUT_PART,
           ...
           'DAY TO HOUR',      TO_VARCHAR(TRUNC(DATEDIFF(SECONDS, SUBTRAHEND, MINUEND)/(86400))) || ' ' || PUBLIC.TIME_DIFFERENCE(LEAST(MINUEND,SUBTRAHEND), GREATEST(MINUEND, SUBTRAHEND), 'HOUR'),
           'DAY TO MINUTE',    TO_VARCHAR(TRUNC(DATEDIFF(SECONDS, SUBTRAHEND, MINUEND)/(86400))) || ' ' || PUBLIC.TIME_DIFFERENCE(LEAST(MINUEND,SUBTRAHEND), GREATEST(MINUEND, SUBTRAHEND), 'MINUTE'),
           'DAY TO SECOND',    TO_VARCHAR(TRUNC(DATEDIFF(SECONDS, SUBTRAHEND, MINUEND)/(86400))) || ' ' || PUBLIC.TIME_DIFFERENCE(LEAST(MINUEND,SUBTRAHEND), GREATEST(MINUEND, SUBTRAHEND), 'SECOND'),
           'HOUR',             TO_VARCHAR(DATEDIFF(HOUR, SUBTRAHEND, MINUEND)),
           'HOUR TO MINUTE',   TO_VARCHAR(DATEDIFF(HOUR, SUBTRAHEND, MINUEND)) || ':' || SPLIT_PART(PUBLIC.TIME_DIFFERENCE(LEAST(MINUEND,SUBTRAHEND), GREATEST(MINUEND, SUBTRAHEND), 'MINUTE'), ':', 2),
           'HOUR TO SECOND',   TO_VARCHAR(DATEDIFF(HOUR, SUBTRAHEND, MINUEND)) || ':' || SUBSTR(PUBLIC.TIME_DIFFERENCE(LEAST(MINUEND,SUBTRAHEND), GREATEST(MINUEND, SUBTRAHEND), 'SECOND'), -5),
           'MINUTE',           TO_VARCHAR(DATEDIFF(MINUTE, SUBTRAHEND, MINUEND)),
           'MINUTE TO SECOND', TO_VARCHAR( TRUNC(DATEDIFF(SECONDS, SUBTRAHEND, MINUEND)/60), 'FM9900') || ':' || SUBSTR(PUBLIC.TIME_DIFFERENCE(LEAST(MINUEND,SUBTRAHEND), GREATEST(MINUEND, SUBTRAHEND), 'SECOND'), -2),
